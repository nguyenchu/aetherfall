// Content: spells, items, and the starting party.
// Kept separate from logic so balancing and content can be changed in one place.
// Enemies live in chapters.ts next to the areas they appear in.

import type { Combatant, Item, Spell, Stats } from './types';

function stats(s: Partial<Stats> & Pick<Stats, 'maxHp' | 'str' | 'agi' | 'vit' | 'int'>): Stats {
  const maxMp = s.maxMp ?? 0;
  return {
    maxHp: s.maxHp,
    hp: s.hp ?? s.maxHp,
    maxMp,
    mp: s.mp ?? maxMp,
    str: s.str,
    agi: s.agi,
    vit: s.vit,
    int: s.int,
  };
}

export const SPELLS: Record<string, Spell> = {
  // Lyra — Hexweaver: elemental damage plus status setup.
  fire: {
    id: 'fire', name: 'Ember Hex', cost: 4, kind: 'damage', power: 14,
    element: 'fire', target: 'enemy', desc: 'Fire hex. May Burn.',
    inflict: { ailment: 'burn', chance: 0.35, rounds: 2 },
  },
  frost: {
    id: 'frost', name: 'Rime Hex', cost: 5, kind: 'damage', power: 16,
    element: 'ice', target: 'enemy', desc: 'Ice hex. May Chill.',
    inflict: { ailment: 'chill', chance: 0.35, rounds: 2 },
  },
  firewave: {
    id: 'firewave', name: 'Hexstorm', cost: 9, kind: 'damage', power: 11,
    element: 'fire', target: 'all-enemies', desc: 'Fire hex on all enemies. May Burn.',
    inflict: { ailment: 'burn', chance: 0.25, rounds: 2 },
  },
  blizzard: {
    id: 'blizzard', name: 'Winter Sigil', cost: 12, kind: 'damage', power: 14,
    element: 'ice', target: 'all-enemies', desc: 'Ice sigil on all enemies. May Chill.',
    inflict: { ailment: 'chill', chance: 0.25, rounds: 2 },
  },
  // Mira — Dawnkeeper: healing, holy pressure, and party sustain.
  smite: {
    id: 'smite', name: 'Dawnstrike', cost: 6, kind: 'damage', power: 18,
    element: 'holy', target: 'enemy', desc: 'Holy damage to one enemy.',
  },
  cure: {
    id: 'cure', name: 'Dawnmend', cost: 4, kind: 'heal', power: 26,
    element: 'none', target: 'ally', desc: 'Heals one ally.',
  },
  cureall: {
    id: 'cureall', name: 'Sunward', cost: 9, kind: 'heal', power: 16,
    element: 'none', target: 'party', desc: 'Heals the whole party.',
  },
  // Kael — Aetherblade: fast physical pressure and guard breaking.
  bash: {
    id: 'bash', name: 'Guardbreak', cost: 3, kind: 'damage', power: 18,
    element: 'physical', target: 'enemy', guardHit: 1,
    desc: 'Aetherblade strike. Always chips 1 guard pip.',
  },
  cleave: {
    id: 'cleave', name: 'Arc Sweep', cost: 4, kind: 'damage', power: 9,
    element: 'physical', target: 'all-enemies', desc: 'Sweeping aether slash on all enemies.',
  },
};

export const ITEMS: Record<string, Item> = {
  potion: {
    id: 'potion',
    name: 'Elixir',
    kind: 'heal',
    power: 30,
    target: 'ally',
    buyPrice: 15,
    sellPrice: 7,
    description: 'Restores 30 HP.',
  },
  tonic: {
    id: 'tonic',
    name: 'Aether Tonic',
    kind: 'mp',
    power: 12,
    target: 'ally',
    buyPrice: 22,
    sellPrice: 11,
    description: 'Restores 12 MP.',
  },
  hi_potion: {
    id: 'hi_potion',
    name: 'Greater Elixir',
    kind: 'heal',
    power: 65,
    target: 'ally',
    buyPrice: 42,
    sellPrice: 20,
    description: 'Restores 65 HP.',
  },
  hi_tonic: {
    id: 'hi_tonic',
    name: 'Aether Concentrate',
    kind: 'mp',
    power: 24,
    target: 'ally',
    buyPrice: 46,
    sellPrice: 22,
    description: 'Restores 24 MP.',
  },
  purifying_draught: {
    id: 'purifying_draught',
    name: 'Purifying Draught',
    kind: 'cure',
    power: 0,
    target: 'ally',
    buyPrice: 25,
    sellPrice: 10,
    description: 'Cures burn, chill, and venom. Battle only.',
  },
  phoenix_down: {
    id: 'phoenix_down',
    name: 'Phoenix Down',
    kind: 'revive',
    power: 0.4,
    target: 'ally',
    buyPrice: 120,
    sellPrice: 45,
    description: 'Revives a fallen ally at 40% HP.',
  },
  mega_potion: {
    id: 'mega_potion',
    name: 'Elixir of Renewal',
    kind: 'heal',
    power: 100,
    target: 'ally',
    buyPrice: 180,
    sellPrice: 70,
    description: 'Restores 100 HP.',
  },
  mega_tonic: {
    id: 'mega_tonic',
    name: 'Aether Wellspring',
    kind: 'mp',
    power: 40,
    target: 'ally',
    buyPrice: 90,
    sellPrice: 40,
    description: 'Restores 40 MP.',
  },
  // Offensive, throwable items — the new type: they target an enemy instead
  // of an ally, dealing flat damage with no MP cost. Battle only, same as cure.
  aether_bomb: {
    id: 'aether_bomb',
    name: 'Aether Bomb',
    kind: 'damage',
    power: 45,
    target: 'enemy',
    buyPrice: 35,
    sellPrice: 15,
    description: 'Hurls unstable Aether at one enemy. Deals 45 damage.',
  },
  frost_vial: {
    id: 'frost_vial',
    name: 'Frost Vial',
    kind: 'damage',
    power: 30,
    target: 'enemy',
    inflict: { ailment: 'chill', chance: 0.5, rounds: 2 },
    buyPrice: 32,
    sellPrice: 14,
    description: 'Shatters into freezing shards. Deals 30 damage, may Chill.',
  },
  tide_pearl: {
    id: 'tide_pearl',
    name: 'Tide Pearl',
    kind: 'sell',
    power: 0,
    target: 'none',
    sellPrice: 12,
    description: 'A damp pearl merchants will buy.',
  },
  wolf_pelt: {
    id: 'wolf_pelt',
    name: 'Wolf Pelt',
    kind: 'sell',
    power: 0,
    target: 'none',
    sellPrice: 9,
    description: 'A shadow wolf pelt merchants will buy.',
  },
  cinder_shard: {
    id: 'cinder_shard',
    name: 'Cinder Shard',
    kind: 'sell',
    power: 0,
    target: 'none',
    sellPrice: 21,
    description: 'A fragment of magma-glass, still warm.',
  },
  prism_shard: {
    id: 'prism_shard',
    name: 'Prism Shard',
    kind: 'sell',
    power: 0,
    target: 'none',
    sellPrice: 25,
    description: 'A shard split from a living geode.',
  },
  warden_sigils: {
    id: 'warden_sigils',
    name: 'Warden Sigils',
    kind: 'sell',
    power: 0,
    target: 'none',
    sellPrice: 28,
    description: 'Old city marks worth selling.',
  },
  rift_shard: {
    id: 'rift_shard',
    name: 'Rift Shard',
    kind: 'sell',
    power: 0,
    target: 'none',
    sellPrice: 40,
    description: 'Torn loose from a collapsing Rift. Merchants pay well for these.',
  },
};

// Color palette for placeholder sprites, ready to be replaced by art later.
const C = {
  warrior: 0x6cf0c2,
  mage: 0x8a6cf0,
  cleric: 0xf0d36c,
} as const;

/** Creates a fresh level 1 party. Saved levels are restored later. */
export function makeParty(): Combatant[] {
  return [
    {
      id: 'kael', name: 'Kael', side: 'party', spriteKey: 'c_kael',
      color: C.warrior, size: 22, spells: ['bash'], level: 1, xp: 0, limit: 0,
      stats: stats({ maxHp: 60, maxMp: 8, str: 14, agi: 8, vit: 10, int: 2 }),
      growth: { maxHp: 10, maxMp: 2, str: 2, vit: 2, agi: 1 },
      learnset: { 4: ['cleave'] },
    },
    {
      id: 'lyra', name: 'Lyra', side: 'party', spriteKey: 'c_lyra',
      color: C.mage, size: 20, spells: ['fire', 'frost'], level: 1, xp: 0, limit: 0,
      stats: stats({ maxHp: 36, maxMp: 18, str: 6, agi: 11, vit: 5, int: 14 }),
      growth: { maxHp: 5, maxMp: 4, int: 2, agi: 1, str: 1, vit: 1 },
      learnset: { 4: ['firewave'], 7: ['blizzard'] },
    },
    {
      id: 'mira', name: 'Mira', side: 'party', spriteKey: 'c_mira',
      color: C.cleric, size: 21, spells: ['cure', 'smite'], level: 1, xp: 0, limit: 0,
      stats: stats({ maxHp: 44, maxMp: 14, str: 9, agi: 7, vit: 8, int: 11 }),
      growth: { maxHp: 7, maxMp: 3, int: 2, str: 1, vit: 1, agi: 1 },
      learnset: { 5: ['cureall'] },
    },
  ];
}
